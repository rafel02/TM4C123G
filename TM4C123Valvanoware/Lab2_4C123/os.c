...
uint32_t SchedCounter; //Thread management counter for Scheduler function
...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	int32_t status;
	status = StartCritical();
  tcbs[4].next = &tcbs[3]; //task0 -> task5
	tcbs[5].next = &tcbs[0]; //task1 -> task2
  SetInitialStack(4); Stacks[4][STACKSIZE-2] = (int32_t)(thread1); // PC to address of thread
  SetInitialStack(5); Stacks[5][STACKSIZE-2] = (int32_t)(thread2); // PC
  EndCritical(status);
  return 1;
}
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	void (*TaskPt)(void); // function pointer variable
	SchedCounter = (SchedCounter + 1) % 100;
	
	//Task0() runs every ms
	TaskPt = (void (*)(void))Stacks[4][STACKSIZE-2]; //take function location from individual stack
	(*TaskPt)();
	
	if(SchedCounter == 0){
		//Task1() runs every 100ms
		TaskPt = (void (*)(void))Stacks[5][STACKSIZE-2]; //take function location from individual stack
		(*TaskPt)();
	}
	
	RunPt = RunPt -> next; //round-robin scheduling
	return;
}
